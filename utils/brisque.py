...
def compute_features(img):
    ...
    for itr_scale in range(scalenum):
        ...
        sigma = np.sqrt(np.maximum(0, sigma - mu_sq))
...
def test_measure_BRISQUE(imgPath, is_img = False, model_path = "allmodel"):
    if is_img:
        dis = imgPath
    else:
    ...
    x, idx = gen_svm_nodearray(x[1:], isKernel=(model.param.kernel_type == svm.PRECOMPUTED))
